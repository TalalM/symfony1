...
class sfMemcachedCache extends sfCache
{
  ...
  */
  ...
  public function set($key, $data, $lifetime = null)
  {
    ...
    if (false !== $this->memcache->replace($this->getOption('prefix').$key, $data, false))
    {
      return true;
    }

    return $this->memcache->set($this->getOption('prefix').$key, $data, false);
  }
  ...
  {
    $values = array();
    foreach ($this->memcache->getMulti(array_map(create_function('$k', 'return "'.$this->getOption('prefix').'".$k;'), $keys)) as $key => $value)
    {
  ...
  {
    $this->memcache->set($this->getOption('prefix').'_metadata'.self::SEPARATOR.$key, array('lastModified' => time(), 'timeout' => time() + $lifetime), false);
  }
